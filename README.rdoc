...
Currently only development on Mac OS X is supported. This will be addressed soon by specifying where to find the executable on your system.
...
* Start the executable with the +-server+ option (or as a daemon) under the user that your application or the gem will be running as e.g.
...
Using the gem in production is not supported yet, but will be soon (i.e. a few days to a week). Setup procedures on a server is similar to that of development so refer to those steps above in order to get your server set up in the meantime.
